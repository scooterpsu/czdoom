...
static inline signed short fudgea(signed short b)
{
  ...
  //  CongoZombie (11/10/09)- functions to aid in the conversion of weapon enums
  //  to weapon slot numbers (keyboard numbers).

int num2wpn(int wpnNum, int pref){
    switch(wpnNum){
       
        case 0:
            printf("Weapon 0 chosen. %i\n", P_WeaponPreferred(wp_chainsaw, wp_fist));
            return (pref)?P_WeaponPreferred(wp_chainsaw, wp_fist): wp_fist;
        break;
        
        case 1:
            printf("Weapon 1 chosen. %i\n", wp_pistol);
            return wp_pistol;
        break;

        case 2:
            printf("Weapon 2 chosen. %i\n", P_WeaponPreferred(wp_supershotgun, wp_shotgun));
            return (pref)?P_WeaponPreferred(wp_supershotgun, wp_shotgun): wp_shotgun;
        break;

        case 3:
            printf("Weapon 3 chosen. %i\n", wp_chaingun);
            return wp_chaingun;
        break;

        case 4:
            printf("Weapon 4 chosen. %i\n", wp_missile);
            return wp_missile;
        break;

        case 5:
            printf("Weapon 5 chosen. %i\n", wp_plasma);
            return wp_plasma;
        break;

        case 6:
            printf("Weapon 6 chosen. %i\n", wp_bfg);
            return wp_bfg;
        break;
        
        default:
            printf("Default case. %i\tSent: %i\n", wp_fist, wpnNum);
            return P_WeaponPreferred(wp_chainsaw, wp_fist);
    }
}

int wpn2num(int wpnNum){
    switch(wpnNum){
...
int hasAmmo(int wpnNum){
    ...
    switch(wpnNum){       
        ...
        case 4:
            return (player->ammo[am_misl]);
        break;

        case 5:
            return (player->ammo[am_cell]);
        break;
...
void G_BuildTiccmd(ticcmd_t* cmd)
{
  ...
  if(gamekeydown[key_weapontoggle]){
  }else{
    if (strafe)
      {
   ...
   //	CongoZombie (11/10/09)- If strafe left and strafe right are held down, don't move
	 //	forward or backwards if up or down is pushed (we want it to
	 //	switch weapon)
  ...
  if ((!demo_compatibility && players[consoleplayer].attackdown &&
       !P_CheckAmmo(&players[consoleplayer])))  //  Disable default weapon toggle functionality.
#else
...
#ifdef HANDHELDMODS
      //  CongoZombie (11/10/09) - GBA style weapon switching:
      //  if strafe left and strafe right are held down together,
      //  weapons can be switched by pressing up and down.
      //  Preferred (best) weapon is chosen with left, and weapon
      //  switches within current type are performed with right.


      //  If both strafe buttons are held down
      if(gamekeydown[key_weapontoggle]){

          const player_t *player = &players[consoleplayer]; // Pointer to the player

          //  Handler for up button
          if(gamekeydown[key_up]){
            int i = wpn2num(player->readyweapon) + 1;
            int done = 0;
            while(!done)
            {

                if(i == 7)    //  If we have gone past the end of the weapons, go back to 0.
                    i = 0;

                if(player->weaponowned[num2wpn(i, 0)] && hasAmmo(i))
                {
                    newweapon = num2wpn(i, 0);
                    done = 1;
                }
                i++;
            }
          ...
          if(gamekeydown[key_down]){
            ...
            while(!done)
            {
                ...
                if(i == -1)    //  If we have gone past the end of the weapons, go back to 6.
                    i = 6;
          ...
          //  If the left button is pressed, switch to the preferred weapon
          if(gamekeydown[key_left]){
             newweapon = P_SwitchWeapon(&players[consoleplayer]);
          }

          //  Allows the switching of different weapon types by pressing right
          //  i.e. Chainsaw to fist or super shotgun to shotgun.
          ...
          if(gamekeydown[key_right]){
              ...
              } else if (player->readyweapon == wp_shotgun || player->readyweapon == wp_supershotgun && gamemode == commercial) {
                  if(player->readyweapon == wp_shotgun && gamemode == commercial){
                      newweapon = (player->weaponowned[wp_supershotgun])? wp_supershotgun: wp_nochange;
                  }else{
                      newweapon = (player->readyweapon != wp_shotgun)? wp_shotgun: wp_nochange;
                  }
              }
          }
...
//  Function to return the current map so that
//  it can be added automatically to the save
//  game name.

int G_GetTotalTime (void)
{
    return totalleveltimes;
}

int G_GetGameMap (void)
{
    return gamemap;
}

int G_GetGameEp (void)
{
    return gameepisode;
}

int G_GetGameSkill (void)
{
    return gameskill;
}
...
static unit_64t G_UpdateSignature(unit_64t s, const char *name)
{
...
static unit_64t G_Signature(void)
{
  static unit_64t s = 0;
...
static void G_DoSaveGame (boolean menu)
{
  ...
  CheckSaveGame(SAVESTRINGSIZE+VERSIONSIZE+sizeof(unit_64t));
