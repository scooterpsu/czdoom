...
void M_SaveConf (int ch)
{
  ...
  //  CongoZombie 12/10/09 - The device doesn't have a keyboard, so we should
  //  automatically fill in the save name with some relevant info so that it 
  //  can be identified later.
  int sTimeElapsed, sSecs, sMins, sHours;
  sTimeElapsed = ((G_GetTotalTime() / 35) +(leveltime / 35));
  sSecs = sTimeElapsed % 60;
  sMins = ((sTimeElapsed - sSecs) % 3600) / 60;
  sHours = (sTimeElapsed - (sMins * 60) - sSecs) / 3600;
    
  if (gamemode == commercial)
      sprintf(savename, "MAP%02d", G_GetGameMap() );
  else
      sprintf(savename, "E%dM%d", G_GetGameEp(), G_GetGameMap() );

  char saveskill[22];
  int skill;
  skill = G_GetGameSkill();

  if(skill != -1)
  {
      switch(skill){
          case 0:
              sprintf(saveskill, " Very Easy %2d:%2d:%2d", sHours, sMins, sSecs );
          break;

          case 1:
              sprintf(saveskill, " Easy %2d:%2d:%2d", sHours, sMins, sSecs );
          break;
          case 2:
              sprintf(saveskill, " Medium %02d:%02d:%02d", sHours, sMins, sSecs );
          break;
          case 3:
              sprintf(saveskill, " Hard %2d:%2d:%2d", sHours, sMins, sSecs );
          break;
          case 4:
              sprintf(saveskill, " Nightmare %2d:%2d:%2d", sHours, sMins, sSecs );
          break;
      }
  }

  strcat(savename, saveskill);

  strcpy(savegamestrings[saveSlot], savename);
  saveCharIndex = strlen(savename);
  M_DoSave(saveSlot);
...
static const char *dogcount[] = { "0", "1", "2", "3"};
...
#else
  {"Number Of Single-Player Helper Dogs",S_CHOICE|S_LEVWARN,m_null,E_X,E_Y+ enem_helpers*8, {"player_helpers"}, 0, 0, NULL, dogcount},
#endif

  // killough 8/8/98
  {"Distance Friends Stay Away",S_NUM,m_null,E_X,E_Y+ enem_distfriend*8, {"friend_distance"}},

  /* Figure how to make this a toggle on dog_jumping 
...
#else
      if (messageNeedsInput == true &&
  !(ch == ' ' || ch == 120 || ch == 115 || ch == key_escape)) // phares
      return false;
#endif
