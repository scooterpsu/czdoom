...
#else
    if ((ev->type == ev_keydown && ev->data1 == key_escape) && !menuactive)
#endif
    {
      AM_Start ();
      rc = true;
...
#ifdef HANDHELDMODS
    else if (ch == key_escape && !menuactive)
#else
    else if (ch == key_map)
#endif
    {
...
#ifdef HANDHELDMODS
    else if ((ch == key_strafeleft) || (ch == key_straferight))
#else
