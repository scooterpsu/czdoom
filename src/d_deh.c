...
boolean deh_GetData(char *, char *, unit_64t *, char **, FILE *);
...
//---------------------------------------------------------------------------
static unit_64t getConvertedDEHBits(unit_64t bits) {
  static const unit_64t bitMap[32] = {
  ...
  int i;
  unit_64t shiftBits = bits;
  unit_64t convertedBits = 0;
  for (i=0; i<32; i++) {
...
//---------------------------------------------------------------------------
static void setMobjInfoValue(int mobjInfoIndex, int keyIndex, unit_64t value) {
  mobjinfo_t *mi;
...
static void deh_procThing(DEHFILE *fpin, FILE* fpout, char *line)
{
  char key[DEH_MAXKEYLEN];
  char inbuffer[DEH_BUFFERMAX];
  unit_64t value;      // All deh values are ints or longs
  int indexnum;
  int ix;
  char *strval;
...
boolean deh_GetData(char *s, char *k, unit_64t *l, char **strval, FILE *fpout)
{
  char *t;  // current char
